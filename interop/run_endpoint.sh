...
#
SUPPORTED="handshake transfer multiconnect transferloss transfercorruption blackhole longrtt amplificationlimit ipv6 keyupdate retry resumption zerortt ecn chacha20 versionnegotiation"
...
if [ "$ROLE" == "client" ]; then
    ...
    if [ "$TESTCASE" == "multiconnect" ]; then
        ...
        # RFC 9000 §6.2: the runner checks the capture for our Initial
        # in a reserved version and the server's matching answer.
        MODE="--negotiate-version --timeout 55"
